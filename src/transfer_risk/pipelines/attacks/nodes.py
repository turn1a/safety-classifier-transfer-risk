...
is wired by surrogate kind at build time: transformers are served from their ONNX graph
(``onnx.{name}``), the BiLSTM from its torch checkpoint (``surrogate.{name}``). TextAttack runs
